...
import { runFlowJob, FlowQuotaError, FlowAutomationError } from "./flowAutomation.ts";
...
export async function runStoryJob(job: StoryJob, onProgress: ProgressCallback): Promise<JobResult> {
  ...
  await onProgress(
    `📹 ${scenePlan.prompts.length} ta video sahna Flow'da generatsiya qilinmoqda. Bu bir necha daqiqa davom etishi mumkin...`
  );
  let sceneResults;
  try {
    sceneResults = await runFlowJob({ jobDir, prompts: scenePlan.prompts });
  } catch (err) {
